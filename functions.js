...
function silentNpmInit() {
  exec('npm init -y', (error, stdout, stderr) => {
    if (error) {
      console.error(`exec error: ${error}`)
      return
    }
    // `stdout` and `stderr` are silenced, so there's nothing to log here.
  })
}
 ...
 */
export async function initialiseCelaris() {
  console.log('Scaffolding Celaris...')

  // Initialise the project with npm
  // await runInteractiveCommand('npm init -y')
  silentNpmInit()
